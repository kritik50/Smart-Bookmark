// BookmarkCard — Individual bookmark card component
// Moved from app/components/BookmarkCard.tsx, now properly typed with React.memo
...
function BookmarkCardInner({
  ...
  return (
    <div
      ...
      className={`group relative bg-white/75 backdrop-blur-md border border-white/70 p-5 rounded-2xl shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1.5 hover:border-indigo-200 transition-all duration-300 cursor-pointer active:cursor-grabbing overflow-hidden ${
    ...
    >
      ...
      {/* Header: favicon + category + action buttons */}
          ...
          {bm.tags && bm.tags.length > 0 && (
            <div className="flex gap-1 overflow-hidden">
              {bm.tags.slice(0, 3).map((tag: string) => (
                <span
                  key={tag}
                  className="text-[9px] font-bold bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200 truncate max-w-[60px]"
                >
                  #{tag}
                </span>
              ))}
...
// React.memo prevents re-renders when parent state changes but this card's props haven't
const BookmarkCard = React.memo(BookmarkCardInner);
